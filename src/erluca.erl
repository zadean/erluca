...
-type yes_no() :: yes | no | string() | binary().
%% Accepted values:
%% ```
...
%% Currently only English as default.
-type lang() :: en.

%% Currently, 10.0 only.
-type version() :: {10, 0}.

-type strength() :: primary | secondary | tertiary | quaternary | identical | 1..5.
%% Accepted values:
%% ```
...
-type max_variable() :: space | punct | symbol | currency | string() | binary().
%% Accepted values:
%% ```
%% space | <<"space">> | "space" |
%% punct | <<"punct">> | "punct" |
...
-type alternate() :: non_ignorable | shifted | blanked | string() | binary().
%% Accepted values:
%% ```
...
-type case_first() :: upper | lower | string() | binary().
%% Accepted values:
%% ```
%% upper | <<"upper">> | "upper" |
%% lower | <<"lower">> | "lower"
...
-type options() ::
    #{
        ...
        version => version(),
        strength => strength(),
        maxVariable => max_variable(),
        alternate => alternate(),
        % reverse second level non-0 values, for e.g. trailing accents in French
        backwards => yes_no(),
        % not implemented
        normalization => no,
        caseLevel => yes_no(),
...
-spec uca(options()) -> fun((unicode:chardata()) -> binary()).
uca(Opts) ->
...
get_fun({en, Strength, MaxVar, Alternate, Backwards, true, CaseFirst}) ->
    Strength1 =
        case lists:member(3, Strength) of
            true -> Strength;
...
get_fun({en, Strength, MaxVar, Alternate, Backwards, false, CaseFirst}) ->
    fun(Str) ->
        ...
        W1 =
            if
                CaseFirst == upper -> upper_first(W);
                true -> W
            end,
        W2 =
            if
                Alternate == blanked -> blank_variables(W1, MaxVar);
                Alternate == shifted -> shift_variables(W1, MaxVar);
                true -> W1
            end,
        ...
        <<
            if
                L == 2 andalso Backwards == yes -> (level_part_key(W2, L, reverse));
                L == 5 -> normalize_to_binary(Str);
                true -> (level_part_key(W2, L, forward))
            end
         || L <- Strength
        >>
    end.
...
parse_options(
    ...
    CaseLevel1 =
        case parse_yes_no(CaseLevel) of
            yes -> true;
    ...
    CaseFirst1 =
        case parse_casefirst(CaseFirst) of
            undefined when Fb -> lower;
            undefined -> throw({error, unknown_case});
            CF -> CF
        end,
    _ =
        case parse_yes_no(Numeric) of
            yes when Fb -> no;
            undefined when Fb -> no;
...
parse_lang(En) when En == en; En == <<"en">>; En == "en" -> en;
...
parse_maxvariable(Max) when Max == space; Max == <<"space">>; Max == "space" -> ?MAXSPACEVAR;
parse_maxvariable(Max) when Max == punct; Max == <<"punct">>; Max == "punct" -> ?MAXPUNCTVAR;
parse_maxvariable(Max) when Max == symbol; Max == <<"symbol">>; Max == "symbol" -> ?MAXSYMBOLVAR;
parse_maxvariable(Max) when Max == currency; Max == <<"currency">>; Max == "currency" ->
    ?MAXCURRVAR;
parse_maxvariable(_) ->
    undefined.

parse_alternate(Alt) when
    Alt == non_ignorable; Alt == <<"non_ignorable">>; Alt == "non_ignorable"
->
    non_ignorable;
parse_alternate(Alt) when Alt == shifted; Alt == <<"shifted">>; Alt == "shifted" -> shifted;
parse_alternate(Alt) when Alt == blanked; Alt == <<"blanked">>; Alt == "blanked" -> blanked;
parse_alternate(_) ->
    undefined.
...
default_options() ->
    #{
        ...
        % reverse second level non-0 values, for trailing accents in French
        backwards => no,
        % not implemented
        normalization => no,
        caseLevel => no,
...
option_keys() -> maps:keys(default_options()).

normalize(Str) ->
    case unicode:characters_to_nfd_list(Str) of
        T when is_tuple(T) -> error;
        B -> lists:flatten(B)
    end.
...
normalize_to_binary(Str) ->
    try
        ...
        _:_ ->
            ...
            if
                S =/= Str -> normalize_to_binary(S);
                true -> error
            end
    end.

strip_controls([I | T]) when
    I =:= 55296;
    I =:= 55297;
    I =:= 55298;
    I =:= 55299;
...
shift_variables([{V1, _, _}, {0, _, W2}, {0, _, W3} | T], Max) when
    V1 >= ?MINVAR andalso
        V1 =< Max andalso
        W2 > 0 andalso
        W3 > 0
...
shift_variables([{V1, _, _}, {0, _, W3} | T], Max) when
    V1 >= ?MINVAR andalso
        V1 =< Max andalso
        W3 > 0
->
    [{0, 0, 0, V1}, {0, 0, 0, 0} | shift_variables(T, Max)];
shift_variables([{V1, _, _} | T], Max) when
    V1 >= ?MINVAR andalso
        V1 =< Max
->
...
blank_variables([{V1, _, _}, {0, _, W2}, {0, _, W3} | T], Max) when
    V1 >= ?MINVAR andalso
        V1 =< Max andalso
        W2 > 0 andalso
        W3 > 0
...
blank_variables([{V1, _, _}, {0, _, W3} | T], Max) when
    V1 >= ?MINVAR andalso
        V1 =< Max andalso
        W3 > 0
->
    [{0, 0, 0, 0}, {0, 0, 0, 0} | blank_variables(T, Max)];
blank_variables([{V1, _, _} | T], Max) when
    V1 >= ?MINVAR andalso
        V1 =< Max
->
    [{0, 0, 0, 0} | blank_variables(T, Max)];
...
upper_first([]) ->
    [];
upper_first([H | T]) when
    not ((element(3, H) >= 8 andalso element(3, H) =< 12) orelse
